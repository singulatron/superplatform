...
func NewDeployService(
	...
	serviceInstanceStore, err := datastoreFactory("deploySvcServiceInstances", &deploy.ServiceInstance{})
	if err != nil {
		return nil, err
	}
	serviceDefinitionStore, err := datastoreFactory("deploySvcServiceInstances", &deploy.ServiceDefinition{})
	if err != nil {
		return nil, err
	}
	nodeStore, err := datastoreFactory("deploySvcNodes", &deploy.Node{})
	if err != nil {
		return nil, err
	}
...
func (ns *DeployService) nodeHeartbeat() {
	first := true
	for {
		if !first {
...
func (ns *DeployService) ParseNvidiaSmiOutput(output string) ([]*deploy.GPU, error) {
...
func (ns *DeployService) getNvidiaSmiOutput() (string, error) {
...
func getResourceUsage() (deploy.ResourceUsage, error) {
	var usage deploy.ResourceUsage
